...
def cubic_interpolation(data: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
    """Compute the control points to interpolates the data using cubic bezier interpolation. It adds control points to the data.
...
def _calculate_control_points(data: List[Tuple[float, float]]) -> list:
    """See:
...
def _create_tri_diagonal_matrix(n):
...
def _create_b_vector(K):
    """This function creates the b vector to find the control points of the cubic bezier curve.
    See "The Vector b" Section in:
    https://exploringswift.com/blog/Drawing-Smooth-Cubic-Bezier-Curve-through-prescribed-points-using-Swift
    """
    n = len(K) - 1
    b = np.array(
